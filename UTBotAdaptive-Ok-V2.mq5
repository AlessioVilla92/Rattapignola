...
//+------------------------------------------------------------------+
//| ENUM — Filtro Ranging (ADX o ER Gate)                            |
//+------------------------------------------------------------------+
// OFF:      nessun filtro, comportamento originale
// ER Gate:  blocca segnali se Efficiency Ratio < soglia (reattivo)
// ADX Gate: blocca segnali se ADX < soglia (stabile, lento)
enum ENUM_RANGE_FILTER
  {
   RANGE_FILT_OFF = 0,  // OFF — nessun filtro ranging
   RANGE_FILT_ER  = 1,  // ER Gate — filtra se Efficiency Ratio < soglia
   RANGE_FILT_ADX = 2,  // ADX Gate — filtra se ADX < soglia
  };
...
input group "                                                               "
input group "╔═══════════════════════════════════════════════════════════╗"
input group "║  📊 FILTRO RANGING                                       ║"
input group "╚═══════════════════════════════════════════════════════════╝"

input ENUM_RANGE_FILTER InpRangeFilter  = RANGE_FILT_OFF;  // ⚙ Tipo filtro ranging
input double            InpERThreshold  = 0.15;            // ER soglia minima (solo se ER Gate)
input int               InpADXPeriod    = 14;              // ADX Period (solo se ADX Gate)
input int               InpADXThreshold = 20;              // ADX soglia minima (solo se ADX Gate)

input group "                                                               "
input group "╔═══════════════════════════════════════════════════════════╗"
input group "║  ⚖️ SENSIBILITA' ASIMMETRICA                             ║"
input group "╚═══════════════════════════════════════════════════════════╝"

input bool              InpDualSens     = false;           // Attiva SELL sensitivity separata
input double            InpKeyValueSell = 1.5;             // Key Value SELL (solo se DualSens)
...
// Handle ADX per filtro ranging (INVALID_HANDLE se filtro non ADX)
int    g_adxHandle = INVALID_HANDLE;

// Dual sensitivity: Key Value SELL effettivo (auto-preset o manuale)
   ...
   switch(preset)
     {
      case TF_PRESET_UT_M1:
         ...
         g_eff_keyValueSell = 0.9;  // ~30% sopra Key 0.7
         break;
      ...
      case TF_PRESET_UT_M5:
         ...
         g_eff_keyValueSell = 1.3;  // ~30% sopra Key 1.0
         break;
      ...
      case TF_PRESET_UT_M15:
         ...
         g_eff_jmaPhase     = 0;    // bilanciato
         g_eff_keyValueSell = 1.5;  // ~25% sopra Key 1.2
         break;
      ...
      case TF_PRESET_UT_M30:
         ...
         g_eff_keyValueSell = 2.0;  // ~33% sopra Key 1.5
         break;
      ...
      case TF_PRESET_UT_H1:
         ...
         g_eff_keyValueSell = 2.5;  // ~25% sopra Key 2.0
         break;
      ...
      case TF_PRESET_UT_H4:
         ...
         g_eff_keyValueSell = 3.0;  // ~20% sopra Key 2.5
         break;
      ...
      default: // MANUAL — l'utente controlla tutto
         ...
         g_eff_keyValueSell = InpKeyValueSell;
         break;
     }
  }
...
int OnInit()
  {
   ...
   if(InpUseBias && _Period != InpBiasTF)
     {
      ...
      // dichiarazione degli input in testa al file (v2.02: 31 parametri).
      ...
      // InpRangeFilter(=OFF), InpERThreshold, InpADXPeriod, InpADXThreshold,
      // InpDualSens(=false), InpKeyValueSell,
      ...
      // InpShowDashboard(=false), InpShowTrailLine(=false)
      g_htfHandle = iCustom(_Symbol, InpBiasTF, "UTBotAdaptive",
                            ...
                            RANGE_FILT_OFF, 0.15, 14, 20,  // range filter OFF (child)
                            false, InpKeyValueSell,         // dual sens OFF (child)
                            false, false,                   // ColorBars OFF, Arrows OFF
                            false, false,                   // ApplyTheme OFF, ShowGrid OFF
                            InpThemeBG,    InpThemeFG,    InpThemeGrid,
                            InpThemeBullCandl, InpThemeBearCandl,
                            false, false,                   // Alert OFF, Push OFF
                            false, false);                  // Dashboard OFF, TrailLine OFF
      if(g_htfHandle == INVALID_HANDLE)
         Print("[UTBot v2.02] WARN: handle HTF bias non valido, bias disabilitato");
     }

   //--- Inizializza handle ADX per filtro ranging ---
   if(InpRangeFilter == RANGE_FILT_ADX)
     {
      g_adxHandle = iADX(_Symbol, PERIOD_CURRENT, InpADXPeriod);
      if(g_adxHandle == INVALID_HANDLE)
         Print("[UTBot v2.02] WARN: ADX handle non valido, filtro ranging disabilitato");
     }
   ...
   Print("[UTBot v2.02] Preset=", EnumToString(InpTFPreset),
         " | Key=", DoubleToString(g_eff_keyValue, 1),
         " | ATR=", g_eff_atrPeriod,
         " | Src=", EnumToString(InpSrcType),
         " | KAMA(", g_eff_kamaN, ",", g_eff_kamaFast, ",", g_eff_kamaSlow, ")",
         " | Range=", EnumToString(InpRangeFilter),
         " | DualSens=", InpDualSens ? "ON(Key=" + DoubleToString(g_eff_keyValueSell, 1) + ")" : "OFF",
         " | Warmup=", g_warmup);
...
void UpdateUTBDashboard(bool forceUpdate = false)
  {
   ...
   //--- HEADER ---
   UTBSetRow(row++, "UTBot v2.02 | " + _Symbol + " | " + EnumToString(_Period),
             C'70,130,255', 10);
   ...
   // Filtro Ranging
   string rfStr;
   switch(InpRangeFilter)
     {
      case RANGE_FILT_OFF: rfStr = "OFF"; break;
      case RANGE_FILT_ER:  rfStr = "ER Gate (>=" + DoubleToString(InpERThreshold, 2) + ")"; break;
      case RANGE_FILT_ADX: rfStr = "ADX Gate (>=" + IntegerToString(InpADXThreshold) +
                                   ", P=" + IntegerToString(InpADXPeriod) + ")"; break;
      default: rfStr = "?"; break;
     }
   UTBSetRow(row++, "Range:    " + rfStr,
             InpRangeFilter == RANGE_FILT_OFF ? C'80,90,110' : C'150,165,185');

   // Dual Sensitivity
   if(InpDualSens)
      UTBSetRow(row++, "DualSens: Key SELL=" + DoubleToString(g_eff_keyValueSell, 1), C'255,180,50');
   else
      UTBSetRow(row++, "DualSens: OFF", C'80,90,110');
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   //--- Legge ADX una sola volta prima del loop (stesso pattern del bias HTF) ---
   double adxValue = 100.0;  // default: no filter (ADX 100 > qualsiasi soglia)
   if(InpRangeFilter == RANGE_FILT_ADX && g_adxHandle != INVALID_HANDLE)
     {
      double adxBuf[1];
   ...
   for(int i = trail_start; i < rates_total; i++)
     {
      ...
      //--- Windowed ER (Kaufman, sempre su close[], tutte le sorgenti) ---
      // v2.02: rimosso proxy single-bar per non-KAMA. ER finestrato sempre.
      double er_val = 0.0;
      ...
      //--- Range filter gate ---
      bool rangePass = true;
      if(InpRangeFilter == RANGE_FILT_ER)
         rangePass = (er_val >= InpERThreshold);
      else if(InpRangeFilter == RANGE_FILT_ADX)
         rangePass = (adxValue >= InpADXThreshold);
      ...
      if(i < rates_total - 1)
        {
         //--- Segnali con filtro bias HTF + range gate ---
         bool isBuy  = (src1 < t1) && (src > trail) && biasLong  && rangePass;
         bool isSell = (src1 > t1) && (src < trail) && biasShort && rangePass;

         //--- Dual Sensitivity (Approach A): SELL richiede distanza extra ---
         if(InpDualSens && isSell)
           {
            double nLossSell = g_eff_keyValueSell * g_atr[i];
            isSell = (t1 - src >= nLossSell - nLoss);
           }
         ...
         //--- Candele colorate ---
         // Barra trigger: GIALLA (2). Ranging: GRIGIO (3). Bull: teal (0). Bear: coral (1).
         if(InpColorBars)
           {
            B_CO[i]   = open[i];
            B_CH[i]   = high[i];
            ...
            if(isBuy || isSell)
               ...
               B_CClr[i] = 3.0;                              // GRIGIO ranging
            else
               B_CClr[i] = (src > trail) ? 0.0 : 1.0;       // teal/coral
           }
        }
      else
        {
         ...
         //--- Barra corrente: colore normale o grigio, NO giallo ---
         if(InpColorBars)
           {
            B_CO[i]   = open[i];
            B_CH[i]   = high[i];
