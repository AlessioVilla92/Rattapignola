...
// --- InpConfirmBars: Conferma persistenza trend ---
// Dopo un crossover (src attraversa trail), il segnale NON viene emesso subito.
// Il prezzo deve restare dalla parte giusta del trail per N barre consecutive.
// Questo filtra i falsi crossover causati da spike/rumore intra-barra.
//   0 o 1 = comportamento originale (segnale immediato al crossover)
//   2     = raccomandato su M5 (conferma in 2 barre = 10 minuti)
//   3+    = piu conservativo, adatto a TF bassi molto rumorosi
// Se il prezzo rientra sotto il trail prima di raggiungere N, il conteggio si resetta.
input int             InpConfirmBars    = 2;        // Barre conferma trend (0/1=off, 2=raccomandato M5)

// --- InpERStrong_Filt / InpERWeak_Filt: Filtro ER Asimmetrico ---
// L'Efficiency Ratio (ER) misura la qualita del trend: 1.0=trend perfetto, 0.0=range puro.
// Questo filtro applica soglie DIVERSE a seconda che il segnale sia:
//   INVERSIONE (cambio direzione, es. da LONG a SHORT): richiede ER >= InpERStrong_Filt
//   CONTINUAZIONE (stessa direzione del trend in corso): richiede ER >= InpERWeak_Filt
// Logica: invertire un trend richiede convinzione forte, continuarlo basta un impulso debole.
// InpERStrong_Filt = 0.0 disattiva il filtro interamente.
// Le soglie 0.35/0.15 coincidono con i livelli ER usati per la colorazione delle frecce.
...
//--- Stato conferma N-barre + trend confermato (Fase 1) ---
// Macchina a stati per la conferma dei segnali:
//   1. Un crossover rawBuy/rawSell imposta pendingDir e count=1
//   2. Ogni barra successiva con trend persistente (src>trail o src<trail) incrementa count
//   3. Quando count >= InpConfirmBars, il segnale viene confermato e lastDir aggiornato
//   4. Se il trend si perde prima di raggiungere N, count e pendingDir si resettano
// lastDir controlla anche il colore delle candele: il trend resta nella direzione
// dell'ultimo segnale confermato finche un nuovo segnale confermato non lo inverte.
// Questo elimina i cambi di colore spurii durante pullback non confermati.
int    g_confirm_pendingDir   = 0;    // direzione in attesa: +1=BUY, -1=SELL, 0=nessuna
int    g_confirm_count        = 0;    // barre consecutive di trend dopo il crossover
int    g_confirm_lastDir      = 0;    // ultima direzione confermata (+1=LONG, -1=SHORT, 0=nessuna)
...
int OnInit()
  {
   ...
   // Necessario su ogni init/cambio parametri per ripartire da zero.
   // lastDir = 0 significa "nessun trend confermato" → le candele useranno
   // il fallback src>trail fino al primo segnale confermato.
...
void UpdateUTBDashboard(bool forceUpdate = false)
  {
   ...
   //--- FILTRI FASE 1 ---
   // Mostra lo stato dei filtri attivi nella dashboard.
   // Confirm: quante barre di conferma richieste, e se c'e un crossover in attesa
   //   mostra [PENDING count/N] per feedback real-time.
   // ER Asym: soglie ER per inversioni (Rev) e continuazioni (Cont).
   if(InpConfirmBars > 1)
      ...
      if(i < rates_total - 1)
        {
         ...
         // Condizione di crossover pura: la sorgente attraversa il trailing stop.
         // rawBuy:  barra precedente era SOTTO trail, barra corrente e SOPRA trail
         // rawSell: barra precedente era SOPRA trail, barra corrente e SOTTO trail
         // Il filtro bias HTF (biasLong/biasShort) e applicato qui a livello grezzo.
         // NOTA: il crossover e un evento puntuale (1 barra). Alla barra successiva
         // src1 sara gia sopra/sotto trail, quindi rawBuy/rawSell tornano FALSE.
         // Per questo la conferma (STEP C) verifica la PERSISTENZA, non la ripetizione.
         ...
         // Questo e intenzionale: l'EA legge B_State per il bias HTF di altre istanze.
         // Il bias deve basarsi sul dato puro — i filtri sono decisioni di trading,
         // non devono inquinare il segnale letto da un'altra istanza su TF superiore.
         if(src1 < t1 && src > t1)
            B_State[i] = 1.0;
         ...
         // Macchina a stati: il crossover (STEP A) dura 1 sola barra.
         // La conferma verifica che il trend PERSISTA per N barre dopo il cross:
         //
         //   Barra i:   rawBuy=TRUE  → pendingDir=+1, count=1
         //   Barra i+1: rawBuy=FALSE, ma src>trail (trend tiene) → count=2
         //   Se count >= InpConfirmBars → confirmedBuy=TRUE, segnale emesso
         //   Se src scende sotto trail prima di N → count=0, reset (falso segnale)
         //
         // InpConfirmBars <= 1: pass-through, comportamento identico all'originale.
         // InpConfirmBars = 2: il crossover deve essere "tenuto" per 2 barre (10 min su M5).
         bool confirmedBuy  = false;
         bool confirmedSell = false;

         if(InpConfirmBars <= 1)
           {
            // Nessuna conferma — segnale immediato al crossover (originale)
            confirmedBuy  = rawBuy;
            confirmedSell = rawSell;
           }
         else
           {
            // Crossover appena avvenuto → inizia conteggio da 1
            if(rawBuy)
              {
            ...
            // Nessun nuovo crossover, ma trend PERSISTE nella direzione pendente:
            // src sopra trail (per BUY) o sotto trail (per SELL) → incrementa contatore
              ...
              {
               // Trend perso prima della conferma (src rientrato dalla parte sbagliata
               // del trail, oppure crossover opposto) → reset, era un falso segnale
               g_confirm_count = 0;
               g_confirm_pendingDir = 0;
              }

            // Emissione: contatore ha raggiunto N → segnale confermato
            if(g_confirm_pendingDir == +1 && g_confirm_count >= InpConfirmBars)
              {
               confirmedBuy = true;
               g_confirm_count = 0;        // reset per il prossimo ciclo
               g_confirm_pendingDir = 0;
              }
            if(g_confirm_pendingDir == -1 && g_confirm_count >= InpConfirmBars)
              {
         ...
         // Applicato DOPO la conferma N-barre, PRIMA dell'emissione finale.
         // Logica asimmetrica:
         //   INVERSIONE (es. da LONG a SHORT): richiede ER >= InpERStrong_Filt (0.35)
         //     → serve trend forte per giustificare un cambio di direzione
         //   CONTINUAZIONE (stessa direzione): richiede ER >= InpERWeak_Filt (0.15)
         //     → basta un impulso minimo per confermare la direzione corrente
         // Se l'ER e sotto soglia, il segnale viene scartato silenziosamente.
         // Il trend (candele) resta nella direzione precedente — nessuna freccia emessa.
         // InpERStrong_Filt = 0.0 disattiva questo filtro interamente.
         if(InpERStrong_Filt > 0.0 && (confirmedBuy || confirmedSell))
           {
            ...
            if(er_val < erThreshold)
              {
               // ER troppo debole: segnale scartato, trend precedente invariato
               confirmedBuy  = false;
               confirmedSell = false;
              }
           }

         // Aggiorna l'ultima direzione confermata SOLO se il segnale e passato.
         // Se bloccato da ER, lastDir NON cambia → il trend resta nella direzione
         // precedente, e il prossimo segnale nella stessa direzione sara ancora
         // valutato come "inversione" (soglia forte) — comportamento corretto.
         if(confirmedBuy)  g_confirm_lastDir = +1;
         if(confirmedSell) g_confirm_lastDir = -1;

         //=== STEP E: Segnali finali ===
         // Alias per chiarezza: isBuy/isSell sono i segnali che hanno superato
         // TUTTI i filtri (conferma N-barre + ER asimmetrico).
         bool isBuy  = confirmedBuy;
         bool isSell = confirmedSell;

         //=== STEP F: Frecce — solo segnali confermati ===
         // Nessuna freccia grigia/bloccata: se il segnale non passa i filtri,
         // semplicemente non viene mostrato. Il chart resta pulito.
         // Il colore della freccia riflette l'ER della barra (erIdx 0-3).
         B_Buy[i]     = isBuy  ? (low[i]  - g_atr[i] * 0.5) : EMPTY_VALUE;
         B_BuyClr[i]  = isBuy  ? (double)erIdx : 0.0;
         B_Sell[i]    = isSell ? (high[i] + g_atr[i] * 0.5) : EMPTY_VALUE;
         B_SellClr[i] = isSell ? (double)erIdx : 0.0;

         //=== STEP G: Entry level line (solo segnali CONFERMATI) ===
         // La linea viola di entry si aggiorna SOLO su segnali confermati.
         // Tra un segnale e l'altro, il livello resta fisso (carry-forward).
         if(isBuy || isSell)
            g_entryLevel = close[i];
         B_EntryLine[i] = g_entryLevel;

         //=== STEP H: Candele colorate — trend confermato ===
         // DIFFERENZA CHIAVE rispetto all'originale:
         // Originale: colore = src > trail (crossover GREZZO, cambia ad ogni micro-pullback)
         // Filtered:  colore = g_confirm_lastDir (ultimo segnale CONFERMATO)
         //
         // Risultato: le candele restano teal durante un uptrend confermato anche se
         // c'e un pullback temporaneo sotto il trail. Il colore cambia SOLO quando
         // un nuovo segnale confermato inverte la direzione.
         //
         // Fallback (g_confirm_lastDir == 0): prima del primo segnale confermato
         // nella sessione, usa il comportamento grezzo src>trail.
         if(InpColorBars)
           {
            B_CO[i]   = open[i];
            B_CH[i]   = high[i];
            ...
            else if(g_confirm_lastDir == +1)
               B_CClr[i] = 0.0;                          // teal — trend LONG confermato
            else if(g_confirm_lastDir == -1)
               B_CClr[i] = 1.0;                          // coral — trend SHORT confermato
            else
               B_CClr[i] = (src > trail) ? 0.0 : 1.0;   // fallback prima del primo segnale
           }
        }
      else
        {
         ...
         //--- Barra corrente: colore trend confermato (NO giallo, mai trigger) ---
         // Stessa logica di STEP H ma senza giallo trigger (la barra aperta
         // non puo generare segnali — anti-repainting).
         // Il colore segue g_confirm_lastDir: il trend in corso resta invariato
         // finche la barra non chiude e non viene eventualmente confermato un nuovo segnale.
         if(InpColorBars)
           {
            B_CO[i]   = open[i];
            B_CH[i]   = high[i];
            B_CL[i]   = low[i];
            B_CC[i]   = close[i];
            if(g_confirm_lastDir == +1)
               B_CClr[i] = 0.0;                          // teal — trend LONG in corso
            else if(g_confirm_lastDir == -1)
               B_CClr[i] = 1.0;                          // coral — trend SHORT in corso
            else
               B_CClr[i] = (src > trail) ? 0.0 : 1.0;   // fallback prima del primo segnale
           }
        }
