//+------------------------------------------------------------------+
//|                                    rattStatePersistence.mqh      |
//|           Rattapignola EA v1.7.2 — State Persistence             |
//|                                                                  |
...
//|                                                                  |
//| Namespace: RATT_STATE_{SYMBOL}_{MAGIC}_{varName}                 |
//| Es: RATT_STATE_EURUSD_12345_sessionProfit                        |
//| Ogni combinazione Symbol+Magic ha il suo namespace isolato,      |
//| permettendo EA multipli sullo stesso terminale.                  |
//+------------------------------------------------------------------+
#define RATT_GV_PREFIX "RATT_STATE_"
...
{
   return RATT_GV_PREFIX + _Symbol + "_" + IntegerToString(MagicNumber) + "_" + varName;
}
...
bool HasSavedState()
{
   ...
   if(!hasOrders)
   {
      AdLogI(LOG_CAT_PERSIST, "Fresh start — no Rattapignola orders. Clearing orphan state.");
      ClearSavedState();
      return false;
   }
...
{
   string prefix = RATT_GV_PREFIX + _Symbol + "_" + IntegerToString(MagicNumber) + "_";
   int deleted = 0;
