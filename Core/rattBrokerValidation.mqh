//+------------------------------------------------------------------+
//|                                      rattBrokerValidation.mqh    |
//|           Rattapignola EA v1.6.1 — Broker Validation             |
//|                                                                  |
...
bool ValidateInputParameters()
{
   ...
   // UTBot Engine: Key Value must be positive
   if(InpKeyValue <= 0)
   {
      Log_SystemError("Validation", 0, StringFormat("InpKeyValue must be > 0 (current: %.2f)", InpKeyValue));
      errors++;
   }
