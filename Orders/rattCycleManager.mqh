//+------------------------------------------------------------------+
//|                                        rattCycleManager.mqh      |
//|           Rattapignola EA v1.7.2 — Cycle Manager                 |
//|                                                                  |
                ...
                const MqlTradeResult& result)
{
   ...
   if(StringFind(dealComment, "RATT_") < 0) return;
...
void CheckExpiry()
{
   ...
   for(int i = 0; i < ArraySize(g_cycles); i++)
   {
      ...
      if(barsElapsed >= g_pendingExpiry)
      {
...
void MonitorActive()
{
   for(int i = 0; i < ArraySize(g_cycles); i++)
   {
      ...
      if(!IsPositionOpen(g_cycles[i].ticket))
      {
         ...
         if(netPL > 0)
         {
            Alert(StringFormat("Rattapignola TP HIT #%d %s | Profit=+%.2f | TP=%s | %s",
                  g_cycles[i].cycleID,
                  g_cycles[i].direction > 0 ? "BUY" : "SELL",
                  netPL, FormatPrice(g_cycles[i].tpPrice), _Symbol));
            AddFeedItem("TP hit " + FormatPrice(g_cycles[i].tpPrice) + " +$" + DoubleToString(netPL, 2), RATT_BUY);
         }
         else
         {
            Alert(StringFormat("Rattapignola CLOSED #%d %s | Loss=%.2f | %s",
                  g_cycles[i].cycleID,
                  g_cycles[i].direction > 0 ? "BUY" : "SELL",
                  netPL, _Symbol));
            AddFeedItem("SL hit -$" + DoubleToString(MathAbs(netPL), 2), RATT_SELL);
         }
...
//+------------------------------------------------------------------+
//| CloseOppositeOnSignal — S2S flip: close/cancel opposite cycles   |
//+------------------------------------------------------------------+
int CloseOppositeOnSignal(int newDirection)
{
   int closed = 0;
   for(int i = 0; i < ArraySize(g_cycles); i++)
   {
      if(g_cycles[i].direction != -newDirection) continue;

      if(g_cycles[i].state == CYCLE_ACTIVE || g_cycles[i].state == CYCLE_HEDGING)
      {
         AdLogI(LOG_CAT_CYCLE, StringFormat("S2S FLIP: Chiudo %s #%d (ticket=%d)",
                g_cycles[i].direction > 0 ? "BUY" : "SELL",
                g_cycles[i].cycleID, g_cycles[i].ticket));

         if(EnableHedge && HsEnabled)
            HsCleanup(i, "S2S_Flip");
         ...
         if(ClosePosition(g_cycles[i].ticket))
         {
            ...
            g_sessionRealizedProfit += soupPL;
            g_dailyRealizedProfit   += soupPL;
            double netPL = g_cycles[i].profit;
            if(netPL > 0) { g_sessionWins++; g_dailyWins++; }
            else          { g_sessionLosses++; g_dailyLosses++; }
      ...
      else if(g_cycles[i].state == CYCLE_PENDING)
      {
         AdLogI(LOG_CAT_CYCLE, StringFormat("S2S CANCEL: Cancello pending %s #%d (ticket=%d)",
...
//+------------------------------------------------------------------+
//| CheckSqzExit — SqueezeMomentum half-peak exit                   |
//+------------------------------------------------------------------+
void CheckSqzExit()
{
   if(g_sqzHandle == INVALID_HANDLE) return;

   double exitLong[1], exitShort[1];
   if(CopyBuffer(g_sqzHandle, 7, 1, 1, exitLong) < 1) return;
   if(CopyBuffer(g_sqzHandle, 8, 1, 1, exitShort) < 1) return;
   ...
   for(int i = 0; i < ArraySize(g_cycles); i++)
   {
      ...
      bool doExit = false;
      if(g_cycles[i].direction == +1 && exitLong[0] == 1.0)  doExit = true;
      if(g_cycles[i].direction == -1 && exitShort[0] == 1.0) doExit = true;

      if(doExit)
      {
         AdLogI(LOG_CAT_CYCLE, StringFormat("SQZ EXIT: Chiudo %s #%d",
                g_cycles[i].direction > 0 ? "BUY" : "SELL", g_cycles[i].cycleID));
...
void MonitorCycles(const EngineSignal &sig)
{
   ...
   CheckExpiry();
   UpdatePending(sig);
   if(InpUseSqzExit) CheckSqzExit();   // BEFORE MonitorActive
   MonitorActive();
}
