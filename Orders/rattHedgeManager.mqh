//+------------------------------------------------------------------+
//|                                      rattHedgeManager.mqh        |
//|           Rattapignola EA v1.7.2 — Hedge Smart Manager           |
//|                                                                   |
...
//|   1. Prossimo segnale UTBot stesso senso Soup (≥ HsAntiWhipsawBars)|
...
//|  COMMENT FORMAT: "RATT_HS_SELL_#12" / "RATT_HS_BUY_#12"         |
//|                                                                   |
//|  API PUBBLICA:                                                     |
//|    HedgeInit()                                                    |
//|    HedgeDeinit()                                                   |
...
#define HS_LINE_PREFIX    "RATT_HS_LINE_"
#define HS_ZONE_TRIGGER   "RATT_HS_ZONE_TRG"
#define HS_ZONE_TP        "RATT_HS_ZONE_TP"
...
void HedgeDeinit()
{
   ...
   for(int i = ObjectsTotal(0) - 1; i >= 0; i--)
   {
      ...
      if(StringFind(name, HS_LINE_PREFIX)  >= 0 ||
         ...
         StringFind(name, "RATT_HS_BE_")   >= 0 ||
         StringFind(name, "RATT_HS_TP_")   >= 0)
      {
         ObjectDelete(0, name);
         removed++;
      }
...
//|                                                                   |
//| Legge la candela che ha generato il segnale UTBot (barra [1]).     |
...
void HsDrawTriggerLine(int slot, double triggerLevel, datetime barTime)
{
   ...
   {
      ObjectSetInteger(0, name, OBJPROP_COLOR,     RATT_HS_TRIGGER_CLR);
...
{
   string name = StringFormat("RATT_HS_BE_%d", g_cycles[slot].cycleID);
   ObjectDelete(0, name);
   ObjectCreate(0, name, OBJ_ARROW, 0, TimeCurrent(), fillPrice);
   ObjectSetInteger(0, name, OBJPROP_ARROWCODE, 4);      // Rombo
   ObjectSetInteger(0, name, OBJPROP_COLOR,     RATT_HS_BE_CLR);
...
{
   string name = StringFormat("RATT_HS_TP_%d", g_cycles[slot].cycleID);
   ObjectDelete(0, name);
   ObjectCreate(0, name, OBJ_ARROW, 0, TimeCurrent(), tpRefLevel);
   ObjectSetInteger(0, name, OBJPROP_ARROWCODE, 4);      // Rombo
   ObjectSetInteger(0, name, OBJPROP_COLOR,     RATT_HS_TP_CLR);
   ObjectSetInteger(0, name, OBJPROP_WIDTH,     2);
   ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, name, OBJPROP_HIDDEN,    true);
   ObjectSetInteger(0, name, OBJPROP_BACK,      false);
...
//|                                                                   |
//| Chiamato da Rattapignola.mq5 dopo CreateCycle().                 |
...
void HsPlaceOrder(int slot, const EngineSignal &sig)
{
   ...
   string comment = StringFormat("RATT_HS_%s_#%d",
   ...
   if(placed)
   {
      ...
      Alert(StringFormat("Rattapignola HS PIAZZATO #%d %s STOP @ %s | Lot=%.2f | %s",
            g_cycles[slot].cycleID,
            g_cycles[slot].direction > 0 ? "SELL" : "BUY",
   ...
   if(!stillPending)
   {
      for(int j = PositionsTotal() - 1; j >= 0; j--)
      {
         ...
         if(StringFind(posComment, StringFormat("#%d", g_cycles[slot].cycleID)) >= 0)
         {
            ...
            Alert(StringFormat("Rattapignola HS ATTIVATO #%d @ %s | Lot=%.2f | %s",
                  g_cycles[slot].cycleID,
                  DoubleToString(fillPrice, _Digits),
...
void HsClose(int slot, string reason)
{
   ...
   if(PositionSelectByTicket(tkt))
   {
      if(g_trade.PositionClose(tkt))
      {
         ...
         Alert(StringFormat("Rattapignola HS CHIUSO #%d | %s | P&L=%+.2f | %s",
...
//|   sig          — segnale UTBot corrente da EngineCalculate()        |
//|   hasNewSignal — true se questa barra ha prodotto un segnale UTBot |
//+------------------------------------------------------------------+
void HsMonitor(int slot, const EngineSignal &sig, bool hasNewSignal)
{
   ...
   // ════════════════════════════════════════════════════════════
   // EXIT 1 — Prossimo segnale UTBot nella stessa direzione della Soup
   ...
   if(hasNewSignal && sig.isNewSignal && sig.direction == g_cycles[slot].direction)
   {
      ...
      {
         AdLogI(LOG_CAT_HEDGE, StringFormat(
            "HS EXIT1: NextUTBSignal #%d | barsActive=%d | dir=%d",
            g_cycles[slot].cycleID, barsActive, sig.direction));
         HsClose(slot, "NextUTBSignal");
         return;
      }
      else
      {
